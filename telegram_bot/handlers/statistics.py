...
from telegram_bot.tg_db.db_controllers.daily_statistic_controller import add_daily_stat
...
def msg_statistic_handler(message:Message):
    run_in_thread(msg_statistic, message)
...
def msg_statistic(message:Message):
    ...
    add_daily_stat( user['id'], stat_type='nya' if text.startswith("/nya") else message.content_type)